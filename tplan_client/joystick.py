...
class HidJoystick:
    ...
    def __init__(self, timeout: float = .05):
        import hid

        self.d_t = timeout  # seconds
    ...
    def next(self):
        ...
        z = self.device.read(max_length=8, timeout_ms=self.d_t * 1000)

        t = time()
        d_t = t - self.last_time if self.last_time else self.d_t
        self.last_time = time()
